...
def tags_and_preds_iterator(tags, preds, tokens=None):
    '''
    return an generator of [tokens, tags, preds].
    '''
    if tokens == None:
        for tag, pred in zip(tags, preds):
            yield ' '.join(['[PAD]', tag, pred])
    else:
        for token, tag, pred in zip(tokens, tags, preds):
            yield ' '.join([token, tag, pred])


def return_report_by_iterator(iterator):
    counts = evaluate(iterator)
